...
ggplot(rats, aes(x = treat, y = time)) +
  geom_point(position = position_jitter(width = 0.1)) +
  stat_summary(fun = mean, geom = "point", colour = "red") +
  stat_summary(fun = mean, geom = "line", aes(group = poison, linetype = poison)) +
  labs(
...
- For poison type III, treatment B shows an lower average survival time for treatment B compared to the overall pattern of means. ## 3. Model time on treat, poison and their interaction in a linear regression. What do the coefficient estimates of interactions indicate? How does this correspond to your observation in #2?
